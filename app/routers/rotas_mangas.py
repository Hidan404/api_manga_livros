...
@rota_mangas.get("/", summary="Listar mangas", description="Retorna uma lista de todos os mangas disponíveis.")
def listar_mangas(db: Session = Depends(get_db)):
    return MangaController.listar(db)


@rota_mangas.get("/{manga_id}", summary="Obter manga", description="Retorna os detalhes de um manga específico.")
...
@rota_mangas.post("/", summary="Criar manga", description="Cria um novo manga com os dados fornecidos.", dependencies=[Depends(require_role("admin"))], status_code=201)
def criar_manga(
    dados: MangaCreate,
...
@rota_mangas.put("/{manga_id}", summary="Atualizar manga", description="Atualiza os dados de um manga específico.", dependencies=[Depends(require_role("admin"))])
def atualizar_manga(
    manga_id: int,
    dados: MangaUpdate,
    db: Session = Depends(get_db),
...
@rota_mangas.get("/{manga_id}/volumes", summary="Listar volumes", description="Retorna uma lista de todos os volumes de um manga específico.")
def listar_volumes(manga_id: int, db: Session = Depends(get_db)):
    return MangaVolumeController.listar_volumes(db, manga_id)


@rota_mangas.get("/{manga_id}/volumes/{numero}", summary="Obter volume", description="Retorna os detalhes de um volume específico de um manga.")
...
@rota_mangas.put("/{manga_id}/volumes/{numero}", summary="Atualizar volume", description="Atualiza os dados de um volume específico de um manga.")
...
@rota_mangas.post("/{manga_id}/volumes", summary="Adicionar volume", description="Cria um novo volume para um manga específico.", status_code=201, dependencies=[Depends(require_role("admin"))])
